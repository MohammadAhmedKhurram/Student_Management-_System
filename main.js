...
function generateId() {
    ...
    do {
        ...
        let r4 = Math.floor(Math.random() * 9 + 1);
        let r5 = Math.floor(Math.random() * 9 + 1);
        id = `${r1}${r2}${r3}${r4}${r5}`;
    ...
    // Edit Student
        ...
        if (studentIndex !== -1) {
            const updatedDetails = await inquirer.prompt([
                {
                    ...
                    name: "Name",
                    filter: (input) => input ? input.replace(/\b\w/g, (char) => char.toUpperCase()) : undefined,
                },
                {
                    type: "input",
            ...
            if (updatedDetails.Name)
                students[studentIndex].name = updatedDetails.Name;
            if (updatedDetails.Age)
                students[studentIndex].age = updatedDetails.Age;
            if (updatedDetails.Course)
                students[studentIndex].courses = updatedDetails.Course;
            saveStudents(students);
